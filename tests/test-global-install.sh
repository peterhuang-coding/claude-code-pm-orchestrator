...
[ -x "$TARGET/bin/claude-yolo-minimax" ] || fail "MiniMax compatibility entrypoint was not installed"
[ "$(readlink "$TARGET/bin/claude-yolo-minimax")" = "$TARGET/bin/claude-yolo" ] || fail "MiniMax compatibility entrypoint does not delegate to Claude YOLO"
...
[ -x "$GLOBAL_BIN/claude-yolo-minimax" ] || fail "shell-visible MiniMax compatibility entrypoint was not installed"
[ "$(readlink "$GLOBAL_BIN/claude-yolo-minimax")" = "$TARGET/bin/claude-yolo-minimax" ] || fail "shell-visible MiniMax compatibility entrypoint does not point to installed alias"
