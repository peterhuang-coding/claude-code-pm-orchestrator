...
SFKEY_BASE_URL=https://relay.example/v1 \
CLAUDE_CODE_MAX_TOOL_USE_CONCURRENCY=6 \
"$LAUNCHER" --name test-session

grep -Fxq 'ANTHROPIC_AUTH_TOKEN=<unset>' "$OUT" || fail "auth token was not cleared"
...
grep -Fxq 'ANTHROPIC_MODEL=glm-5.2' "$OUT" || fail "main model was not pinned"
grep -Fxq 'ANTHROPIC_DEFAULT_HAIKU_MODEL=glm-5.2' "$OUT" || fail "Haiku model was not pinned"
grep -Fxq 'ANTHROPIC_DEFAULT_SONNET_MODEL=glm-5.2' "$OUT" || fail "Sonnet model was not pinned"
grep -Fxq 'ANTHROPIC_DEFAULT_OPUS_MODEL=glm-5.2' "$OUT" || fail "Opus model was not pinned"
